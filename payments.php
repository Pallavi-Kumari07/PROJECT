...
// For payment details :
$c4 = "SELECT * FROM Bookings B JOIN Payments P ON B.booking_id=P.booking_id WHERE B.passenger_id = :passenger_id";
$st4 = $dbo->conn->prepare($c4);
$st4->bindParam(':passenger_id', $current_user, PDO::PARAM_STR);
$st4->execute();
...
<nav class="miniline"></nav>
    ...
    <br><br><br><br><br>
    <div class="miniline" style="color: rgb(159, 157, 157); font-size: x-large; opacity: 0.85">Payment Details :</div>
    <br><br><br><br>
    <div class="container">
        <div class="cont">
        <table border="1">
            ...
            <tbody>
            <?php if (!empty($pay)): ?>
                <?php foreach ($pay as $pa): ?>
                <tr>
                    <td>
                        <p><?php echo htmlspecialchars($pa['booking_id']); ?></p>
                    </td>
                    <td>
                        <p><?php echo htmlspecialchars($pa['payment_date']); ?></p>
                    </td>
                    <td>
                        <p><?php echo htmlspecialchars($pa['amount']); ?></p>
                    </td>
                    <td>
                        <p><?php echo htmlspecialchars($pa['payment_method']); ?></p>
                    </td>
                </tr>
                <?php endforeach; ?>
                <?php else: ?>
                <tr>
                    <td colspan="8">No payment details found!</td>
                </tr>
            <?php endif; ?>
            </tbody>
        </table>
